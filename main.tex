...
will reset the election timer running on the follower. If the timer runs out because the follower did not 
...
The timeout has to be random to avoid split votes where multiple nodes switch into candidate mode at the same time and send out vote messages 

\subsubsection*{Candiate and leader timer}
This timer replaces the election timer when a node is in candidate or leader state.
It s
