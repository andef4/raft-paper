...
receivedVotes[] & Vote responses per node id, a vote could be granted or rejected \\ \hline
\end{tabular}
...
\textbf{Request handling}
As in the VoteRequest request and response handling, the first action is to check if the term of the
leader in the request is smaller than currentTerm of the current node. If yes, a response with success
set to false is sent back and the leader will immediatly step down.

* prevLogIndex and prevLogTerm check (if missing => false, master will retry. If invalid => purge)
* leaderCommit => update own commitIndex
* entries
...
\begin{tabularx}{\textwidth}{  p{150px}  X  }
  ...
  \def\svgwidth{150}
  \input{scenarios/election/40.pdf_tex}
...
\begin{tabularx}{\textwidth}{  p{150px}  X  }
  \noindent\parbox[c]{\hsize}{
  \def\svgwidth{150}
  \input{scenarios/election/60.pdf_tex}
  } & TODO
\end{tabularx}
...
\begin{tabularx}{\textwidth}{  p{150px}  X  }
  \noindent\parbox[c]{\hsize}{
  \def\svgwidth{150}
  \input{scenarios/election/65.pdf_tex}
  } & TODO
\end{tabularx}
...
\begin{tabularx}{\textwidth}{  p{150px}  X  }
  \noindent\parbox[c]{\hsize}{
  \def\svgwidth{150}
  \input{scenarios/election/72.pdf_tex}
  } &TODO
\end{tabularx}
...
State locking
100% understanding of the paper is required
