...
The official GitHub Page \cite{raft_github} contains many further references about the algorithm, a simulator for the algorithm from which screenshots have been used in this paper and an extensive list of implementations in many different programming languages.
...
This data has to be saved on disk before replying to the RPC call from the leader. The fsync or equivalent system call should be used on the file descriptor to make sure the data has arrived on the disk and not just the write caches of the operating system.
...
The first message pair is RequestVote which is used in the voting process. The request is sent out only by nodes in candidate state.
...
\subsection{RequestVote RPC call}
...
\subsection{AppendEntries RPC call}
The AppendEntries RPC call is used by the leader as a heartbeat message and also for
...
Once again, the cluster starts up without an elected leader. This time, one of nodes is offline which will (with the right timing) provoke a split vote.
}
...
The leader receives new data from a client and writes it into its own log[], without updating his commitIndex. Next, the leader sents out AppendEntry messages with the new data.
}

\raftscopedata{scenarios/data_write/11.pdf_tex}{
The follower nodes write the data into their own log[]. This data is not yet considered committed. The node sends back a success AppendEntry message to the leader.
}

\raftscopedata{scenarios/data_write/29.pdf_tex}{
The leader received enough AppendEntry responses from followers and increases his commitIndex to the newly written data (the line around the data cell is now solid). The leader sends out an AppendEntry heartbeat message with the updated commitIndex 
}

\raftscopedata{scenarios/data_write/34.pdf_tex}{
The followers receive the heartbeat message and update their own commitIndex to the index from the leader, sent in the leaderCommit field of the AppendEntry heartbeat.
}
...
An example for a problem showing up in a real-world implementation which is very find and check for is inconsistent/stale reads. As shown in the "Network partition" scenario above, a Raft cluster can have two active leaders.
It is possible that a client on the majority part of the network updates a value in the Raft log. At the same time a client reading from the other partition can read the old value.
A fix for this problem is that the leader has to wait for a majority of the nodes to acknowledge the leaders heartbeat request, before the leader answers the client. This of course
a big performance degradation for something that happens so seldom. The problem showed up both in etcd and Consul \cite{aphyr_call_me_maybe}, and was solved by providing a flag on the
request which can be set if this problem has to be avoided at all cost \cite{etcd_consistency} \cite{consul_consistency}.
